...
$('document').ready(function(){
		function createGlitter() {
			const glitter = $('<div class="glitter"></div>');
			...
			glitter.css('animation', `glitterFall ${duration}ms linear forwards`);
			setTimeout(() => { glitter.remove(); }, duration);
		}

		// Генерируем новые блестки каждые 200 мс
	...
	$('#bannar_coming').click(function(){ 
		...
		text.split('').forEach((char, index) => {
			if(char === ' ') {
				const span = $('<span>').addClass('birthday-text-space');
				span.css('animation-delay', `${index * 0.2}s`);
				container.append(span);
			} else {
				const span = $('<span>').text(char).addClass('birthday-text-letter');
				span.css('animation-delay', `${index * 0.2}s`);
				container.append(span);
			}
		});
		for (let i = 0; i < 50; i++) {
			let glitter = $('<div class="glitter"></div>');
			glitter.css({
				left: Math.random() * $(window).width(),
				top: -10,
			...
			$('body').append(glitter);
			glitter.css('animation', `glitterFall ${3 + Math.random() * 2}s linear forwards`);
	...
	$('#wish_message').click(function(){
		...
		// Если экран маленький — уменьшаем расстояние между шарами
		if ($(window).width() <= 768) {
